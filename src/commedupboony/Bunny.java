...
import commedupboony.BunnyComms;
...
public abstract class Bunny extends Robot {
    ...
    commedupboony.BunnyComms comms = new BunnyComms(rc, this);


    public Bunny(RobotController rc) throws GameActionException {
        super(rc);
        destination = commedupboony.Util.getRandomMapLocation();
    }
    ...
    public void run() throws GameActionException {
        ...
        if(comms.waitingForMap){ // don't move if we're waiting to receive a map from a tower
            commedupboony.Util.log("Bunny @ " + rc.getLocation() + ". Pausing movement because I'm waiting for a map!");
        }
        else {
            moveLogic();
        }
    ...
    public void updateNearestAlliedPaintTowerLoc() throws GameActionException {
        for (RobotInfo bot : nearbyFriendlies) {
            if (!commedupboony.Util.isTower(bot.getType())) {
                continue;
            }
            ...
            commedupboony.Util.log("Bunny " + rc.getID() + " at " + rc.getLocation() +  " found a tower nearby " + bot.getLocation());
            ...
            if (commedupboony.Util.isPaintTower(bot.getType()) &&
