...
public abstract class Bunny extends Robot {
    ...
    MapLocation[] knownAlliedTowerLocs = new MapLocation[25];
    TowerType[] knownAlliedTowerTypes = new TowerType[25];
...
//        if(rc.getRoundNum() == 260) comms.describeWorld();
    ...
    public void addInKnownTowerIfNotAlreadyIn(MapLocation alliedTowerLocation, TowerType towerType) {
        ...
        for (int i = 0; i < knownAlliedTowerLocs.length; i++) {
            ...
            if (alliedTowerLocation.equals(knownAlliedTowerLocs[i])) {
                alreadyIn = true;
            }
        }
        if (!alreadyIn && nullIdx != -1) {
            knownAlliedTowerLocs[nullIdx] = alliedTowerLocation;
            knownAlliedTowerTypes[nullIdx] = towerType;
        }
    }
