...
//        if(true){
    ...
    public static int[] getSectorAndNeighborsExact(MapLocation loc, int sectorsAway) {
        // TODO: bad, don't use
        int sectorRows = (rc.getMapWidth() + 4) / 5;
        int sectorCols = (rc.getMapHeight() + 4) / 5;
        ...
        // Precompute bounds to avoid repeated checks
        int minRow = row - sectorsAway;
        int maxRow = row + sectorsAway;
        int minCol = col - sectorsAway;
        int maxCol = col + sectorsAway;

        // Calculate the number of valid neighbors
        int neighborCount = 8*sectorsAway;
