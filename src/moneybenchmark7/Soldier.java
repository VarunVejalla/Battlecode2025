...
//        updateExplorationType();
    ...
    public MapLocation checkForCloserRuinToBuild(int distance) throws GameActionException {
        // Spirals outward up to vision radius.
        // 1500 bytecode.
        for(int index : spiralOutwardIndices) {
            if (nearbyMapInfos[index] != null && nearbyMapInfos[index].getMapLocation().distanceSquaredTo(myLoc) >= distance) {
                return null;
            }
    ...
    public void buildPatternHardExplore() throws GameActionException {
        ...
        if (currRuinLoc != null) {
            ...
            MapLocation closerRuin = checkForCloserRuinToBuild(myLoc.distanceSquaredTo(currRuinLoc));
            if (closerRuin != null) {
                secondaryDestination = currRuinLoc;
                currRuinLoc = closerRuin;
            }
        }
        ...
        if(currRuinLoc == null){
            if (secondaryDestination != null) {
                currRuinLoc = secondaryDestination;
